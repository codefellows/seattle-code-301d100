...
class App extends React.Component {
  ...
  putCat = async (catObj) => {
    try {
      ...
      // TODO: build the url for axios
      let url = `${process.env.REACT_APP_SERVER}/cats/${catObj._id}`

      // TODO: have axios send the info on cat to update
      let updatedCatFromAxios = await axios.put(url, catObj);

      // TODO: update state with the updated cat
      let updatedCatArray = this.state.cats.map(existingCat => {
        return existingCat._id === catObj._id
        ? updatedCatFromAxios.data
        : existingCat
      });
      ...
      <>
        ...
        <main>
          ...
          <UpdateCatModal
            ...
            catToUpdate={this.state.catToUpdate}
            putCat={this.putCat}
          />
